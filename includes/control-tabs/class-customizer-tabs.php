...
class Customizer_Tabs_Control extends WP_Customize_Control {
	...
	/**
	 * Customize_Control_Tabs constructor.
	 *
	...
	public function render_content() {
		...
		foreach ( $this->tabs as $value => $args ) {
			if ( ! empty( $args['controls'] ) && ( $this->tab_has_controls( $args['controls'] ) ) ) {
				...
				if ( ! empty( $args['icon'] ) ) {
					$output .= '<i class="fab fa-' . esc_attr( $args['icon'] ) . '"></i>';
				}
				if ( ! empty( $args['nicename'] ) ) {
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'tabs-control-script', plugin_dir_url(__FILE__) . 'js/script.js', array( 'jquery' ), TABS_VERSION, true );
		wp_enqueue_style( 'tabs-control-style', plugin_dir_url(__FILE__) . 'css/style.css', null, TABS_VERSION );
	...
	public function partials_helper_script_enqueue() {
		wp_enqueue_script( 'tabs-addon-script', plugin_dir_url(__FILE__) . 'js/customizer-addon-script.js', array( 'jquery' ), TABS_VERSION, true );
	}
